...
### 9.1 Odds-API.io 免费版多公司限制

当前系统使用 Odds-API.io 免费 API 层，该层存在以下限制:

- 每个请求最多返回 2 家博彩公司的赔率数据
- 支持的博彩公司为 Bet365 和 William Hill（由 API 决定，不可配置）
- 每小时请求上限 100 次

**影响**: 虽然 `DEFAULT_BOOKMAKERS` 列表中包含 Bet365、William Hill、Ladbrokes、Unibet、Pinnacle 等 9 家公司，
但免费 API 实际只能返回前 2 家，导致:
- 当前比赛实际可用公司数为 2 家，达不到 >= 3 家的目标
- 分歧指标基于 2 家公司计算，代表性有限
- 共识概率的信源基础较窄
...
| 方案 | 说明 | 成本/工作量 |
|------|------|------------|
| 升级至 Odds-API.io 付费套餐 | 取消 2 家公司限制，可获取全部 9 家数据 | 💰 月费 |
| 增加第二免费 API 作为补充 | 如 the-odds-api.com 免费层，通过 unified.js 聚合多源 | 中 (1-2天代码) |
| 爬虫补充 | 对特定公司官网做轻量爬取 | 高 (需持续维护) |
| 维持现状 | 接受 2 家公司数据，标记 degraded | 0 |

当前状态: 维持现状，在分歧 Tab 中标记 availableBookmakers 数。
